...
export default async function handler(req, res) {
  ...
  try {
    ...
    try {
      // Call your new Vercel Python function
      const transcriptApiUrl = `/api/youtube-transcript?videoId=${videoId}`;
      const transcriptResponse = await fetch(transcriptApiUrl);
      const data = await transcriptResponse.json();

      if (transcriptResponse.ok && data.transcript) {
        transcriptText = data.transcript;
        // Trim extremely long transcripts
        if (transcriptText.length > 16000) transcriptText = transcriptText.slice(0, 16000);
        usedTranscript = true;
      } else {
        console.warn("Transcript fetch failed:", data.error || "Unknown error from Python function");
      }
    } catch (e) {
