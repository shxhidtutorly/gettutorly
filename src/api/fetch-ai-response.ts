...
async function fetchAIResponse(prompt: string): Promise<string> {
  const providers = [
    ...
    { name: "Together", url: "https://api.together.xyz/v1/chat/completions", key: process.env.TOGETHER_API_KEY },
    { name: "Claude", url: "https://api.anthropic.com/v1/messages", key: process.env.CLAUDE_API_KEY },
    { name: "HuggingFace", url: "https://api-inference.huggingface.co/models/tiiuae/falcon-7b", key: process.env.HUGGINGFACE_API_KEY },
    { name: "Gemini", url: "https://generativelanguage.googleapis.com/v1beta/models/gemini-pro:generateContent", key: process.env.GEMINI_API_KEY }
  ];
  ...
  for (const provider of availableProviders) {
    ...
    while (retryCount <= maxRetries) {
      try {
        ...
        // Handle different API structures for different providers
        if (provider.name === "Gemini") {
          ...
          // Default format for OpenAI-compatible APIs
          headers["Authorization"] = `Bearer ${provider.key}`;
          body = JSON.stringify({
