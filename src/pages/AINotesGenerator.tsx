...
const AINotesGenerator = () => {
  ...
  // Inside your AINotesGenerator component...
  ...
  const generateNotes = useCallback(async (text: string, filename: string) => {
    ...
    // This function now correctly calls your new, robust aiNotesService
    try {
      // Set up a fake progress interval for better UX
      const progressInterval = setInterval(() => {
        setProgress(p => Math.min(p + Math.random() * 5, 90));
      }, 500);

      // Call the single, powerful function from the service
      ...
      // Set the state to render the new notes
      setNote(generatedNote);
      setActiveTab('note'); // Reset to the main notes tab
      
      // Track history and show success toast
      await addHistoryEntry(`Source: ${filename}`, JSON.stringify(generatedNote.content.summary, null, 2), { title: generatedNote.content.title, type: 'notes_generation' });
      trackNoteCreated();
      ...
      setIsLoading(false);
      // Let the progress bar finish its animation before hiding it
