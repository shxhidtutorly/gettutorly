...
import { ArrowUp, Paperclip, Square, X, StopCircle, Mic, Globe, BrainCog, FolderCode, LayoutDashboard } from "lucide-react";
import { motion, AnimatePresence } from "framer-motion";
import { useState, useEffect, useRef, useCallback, ElementType, ComponentPropsWithoutRef } from "react";
...
interface PromptInputTextareaProps {
  ...
  placeholders?: string[]; // Changed to array for dynamic placeholders
}
const PromptInputTextarea: React.FC<PromptInputTextareaProps & React.ComponentProps<typeof Textarea>> = ({
  className,
  onKeyDown,
  disableAutosize = false,
  placeholders = ["Type your message here..."], // Default placeholder
  ...props
}) => {
  ...
  useEffect(() => {
    if (placeholders.length > 1) {
      ...
      setCurrentPlaceholderIndex(0); // Reset to first placeholder if only one
    }
...
const [isTypingHeading, setIsTypingHeading] = useState(messages.length === 0);
  ...
  const handleBackToDashboard = () => {
    ...
    setFiles([]);
    setFilePreviews({});
    setIsTypingHeading(true); // Restart typing effect for new session
  };
          ...
          >
            ...
            >
              <PromptInputTextarea
                placeholders={
                  messages.length > 0
                    ? ["Type your next message..."] // Placeholder changes after first message
                    : showSearch
                    ? ["Search the web..."]
                    : showThink
                    ? ["Think deeply..."]
