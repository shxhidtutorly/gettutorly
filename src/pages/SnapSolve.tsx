...
const SnapSolve = () => {
  ...
  const processImage = async () => {
    ...
    try {
      ...
      if (typeof window.Tesseract === 'undefined') {
        throw new Error('Tesseract.js not loaded. Please refresh the page and try again.');
      }
  ...
  const getStepIcon = (step: typeof currentStep) => {
    switch (step) {
      ...
      case "ocr": return <FileImage className="h-5 w-5" />;
  ...
  const getStepTitle = (step: typeof currentStep) => {
    switch (step) {
      ...
      case "ai": return "Getting AI Answer...";
        ...
        <div className="container max-w-4xl mx-auto">
          ...
          <div className="mb-6 md:mb-8">
            <div className="flex justify-between items-center mb-4">
              {["upload", "ocr", "ai", "complete"].map((step, index) => (
                <div key={step} className={`flex items-center ${index < 3 ? 'flex-1' : ''}`}>
                  ...
                  {index < 3 && (
                    <div className={`flex-1 h-1 mx-2 md:mx-4 rounded transition-colors ${
                      index < ["upload", "ocr", "ai", "complete"].indexOf(currentStep)
                        ? 'bg-primary'
                        : 'bg-muted'
                    }`} />
            ...
            <div className="text-center">
              ...
              {isProcessing && (
                <Progress value={progress} className="mt-2 max-w-md mx-auto" />
              )}
            </div>
          </div>
                ...
                <Card className="border-green-200 dark:border-green-800">
                  ...
                  <CardContent>
                    <div className="p-4 md:p-6 bg-green-50 dark:bg-green-950/30 rounded-lg">
                      <p className="whitespace-pre-wrap text-sm md:text-base leading-relaxed">{aiAnswer}</p>
                    </div>
